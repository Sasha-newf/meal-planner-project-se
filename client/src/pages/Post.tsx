...
export default function Post() {
  ...
  async function handleAddToPlan(date: string, mealType: MealType) {
    ...
    try {
      const res = await api.post("/plan", { recipeId: post.recipe.id, date, mealType });
            ...
            <div style={{ display: "grid", gridTemplateColumns: "130px repeat(7, 1fr)", border: "1px solid #e5e7eb", borderRadius: "24px", overflow: "hidden" }}>
              ...
              {weekData.map((day) => (
                <div key={day.fullDate} style={{ padding: "20px 12px", textAlign: "center", background: day.isToday ? "#ecfdf3" : "#fff", borderBottom: "1px solid #e5e7eb" }}>
                  <p style={{ margin: 0, color: day.isToday ? "#16a34a" : "#9ca3af", fontWeight: 800, letterSpacing: "0.08em", textTransform: "uppercase" }}>{day.day}</p>
                    ...
                    return (
                      <div key={`${day.fullDate}-${mealType}`} style={{ minHeight: "120px", padding: "12px", background: day.isToday ? "#f0fdf4" : "#fff", borderTop: mealIdx === 0 ? "none" : "1px solid #e5e7eb" }}>
                        <button
                          ...
                          style={{ width: "100%", height: "100%", borderRadius: "22px", border: items.length ? "2px solid #bbf7d0" : "2px dashed #e5e7eb", background: items.length ? "#f0fdf4" : "white", color: items.length ? "#16a34a" : "#cbd5e1", fontSize: items.length ? "14px" : "28px", cursor: "pointer", padding: "10px", fontWeight: 700 }}
                        >
      ...
      {!post.recipe ? (
        ...
        <>
          ...
          <div style={{ display: "flex", gap: 12, marginBottom: "20px" }}>
            <div style={{ background: "#f5f5f5", padding: "8px 16px", borderRadius: "8px" }}>⏱️ {post.recipe.timeMinutes} min</div>
            <div style={{ background: "#f5f5f5", padding: "8px 16px", borderRadius: "8px" }}>🍽 {post.recipe.servings} servings</div>
          </div>
